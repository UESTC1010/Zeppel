...
class Attention(nn.Module):
    ...
    def forward(
        ...
        elif encoder_hidden_states is not None and not self.is_cross_attention:
            # 修改处
            # using pseduo self attention
            query, key, value = self.c_attn(hidden_states).split(self.split_size, dim=2)
            assert (
                encoder_hidden_states.shape[-1] == key.shape[-1]
            ), "encoder_hidden_states must have the same dim as decoder model dim."
            key = torch.cat([encoder_hidden_states, key], dim=1)
            value = torch.cat([encoder_hidden_states, value], dim=1)
            if attention_mask == None:
                # 预测时，一般没要attention_mask，但是这里必须有，因为必须要encoder_attention_mask，确保和训练时一致。
                attention_mask = -torch.zeros(query.shape[0], 1, 1, query.shape[1]).to(
...
class Block(nn.Module):
    ...
    def forward(
        ...
        # 修改处，有encoder_hidden_states且没新增crossattention时，执行pseudo self attention
        if encoder_hidden_states is not None and not hasattr(self, "crossattention"):
            attn_outputs = self.attn(
                self.ln_1(hidden_states),
                layer_past=layer_past,
                attention_mask=attention_mask,
                encoder_hidden_states=encoder_hidden_states,  # 添加
                encoder_attention_mask=encoder_attention_mask,  # 添加
                head_mask=head_mask,
                use_cache=False,  # 强制为False
        ...
        # 修改处，有encoder_hidden_states并且有crossattention才执行cross attention
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        # 修改处，只要有encoder_hidden_states，无论有没有新增crossattention都执行if内代码进行encoder_attention_mask的生成转换。
